...
\textbf{Query q02} (``legal compliance checking NLP automated verification''): O BM25 obtém os melhores resultados nesta consulta porque os termos ``compliance'', ``checking'' e ``verification'' aparecem literalmente nos títulos dos artigos relevantes. O KNN se aproxima, recuperando artigos com vocabulário adjacente (``conformity'', ``audit''). Cenário ideal para o matching exato do esparso.

\textbf{Query q14} (``lei 14133 compras públicas tecnologia''): Consulta inteiramente em português. O BM25 recupera artigos que mencionam explicitamente a Lei 14.133/2021, incluindo trabalhos sobre inovações da nova lei de licitações e ESG em contratações públicas. O RRF agrega valor ao trazer, via componente KNN, artigos sobre governo digital e transformação tecnológica na administração pública que não contêm o número da lei no texto mas tratam do mesmo contexto.

\textbf{Query q10} (``semi-supervised classification legal text few labels''): Consulta diretamente alinhada ao tema de mestrado do autor. O BM25 recupera artigos sobre aprendizado semissupervisionado e sobre classificação de textos legais, mas nem sempre na intersecção dos dois. O RRF combina resultados: o BM25 encontra os que mencionam ``semi-supervised'' literalmente e o KNN encontra os que discutem ``few-shot'' e ``low-resource'' (termos semanticamente próximos, sem match exato). Este é o cenário onde a fusão se justifica com clareza.

\section{Discussão}

\subsection{Trade-offs entre recuperação esparsa e densa}

\textbf{RRF supera os componentes individuais em todas as métricas de topo.} No corpus bilíngue com fontes heterogêneas (OpenAlex EN, OpenAlex PT, BDTD), o RRF atinge P@10=0,727 e MAP=0,867, superando tanto o BM25 (P@10=0,707, MAP=0,823) quanto o KNN (P@10=0,700, MAP=0,774). A fusão de rankings complementares agrega valor quando há diversidade entre os recuperadores: o BM25 captura termos exatos (e.g., ``14133'', ``procurement'') e o KNN captura proximidade semântica entre documentos que compartilham vocabulário temático sem coincidência exata de termos.
...
\begin{itemize}
    ...
    \item \textbf{Claude/Kiro (Anthropic):} utilizado para apoio na implementação de scripts de coleta e pipeline de recuperação, sugestões de hiperparâmetros e apoio à estruturação do relatório.
\end{itemize}
